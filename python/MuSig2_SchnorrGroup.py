...
class SignScheme:
    def __init__(self, G, nb_participant, nb_nonces):
        ...
        self.Signers = [Signer(G, nb_nonces)
    ...
    def Sign(self):
        ...
        #on calcule les ai
        a = [(int.from_bytes(hl.sha256(b''.join([sum(L).to_bytes(sum(L).bit_length()//7, 'big'),
              L[i].to_bytes(L[i].bit_length()//7, 'big')])).digest(), 'big') % G.order) for i in range(nb_participant)]
        ...
        for j in range(1, nb_nonces):
            b[j] = int.from_bytes(hl.sha256(b''.join([bytes(j), Xtilde.to_bytes(Xtilde.bit_length()//7, 'big')] + [Rn[i].to_bytes(
                Rn[i].bit_length()//7, 'big') for i in range(len(Rn))] + [bytearray(M, 'utf-16')])).digest(), 'big') % G.order
        ...
        #on calcule c
        c = int.from_bytes(hl.sha256(b''.join([Xtilde.to_bytes(Xtilde.bit_length()//7, 'big'), Rsign.to_bytes(
            Rsign.bit_length()//7, 'big'), bytearray(M, 'utf-16')])).digest(), 'big') % G.order
        self.c = c
