...
class SignScheme:
    def __init__(self, nb_participant, nb_nonces):
        ...
        self.Signers = [Signer(nb_nonces) for i in range (nb_participant)] #(Alice, Bob)
    ...
    def Sign(self):
        ...
        R = [[E]*nb_nonces for i in range(nb_participant)] #R[i][j] is Rij. Rij est (Rij.x, Rij.y) (attention deux coordonnées)
        ...
        #on calcule les ai
        a = [ (int.from_bytes(hl.sha256(b''.join([(L[l].x.val).to_bytes(N_bytes,'big') for l in range(nb_participant)] + [(L[i].x.val).to_bytes(N_bytes,'big')])).digest(),'big')%n) for i in range(nb_participant)] 
        ...
        for j in range(1,nb_nonces):
            b[j] = int.from_bytes(hl.sha256(b''.join([bytes(j),(Xtilde.x.val).to_bytes(N_bytes,'big')] + [(Rn[i].x.val).to_bytes(N_bytes,'big') for i in range(len(Rn))] + [bytearray(M,'utf-16')])).digest(),'big')%n
        ...
        #on calcule c
        c = int.from_bytes(hl.sha256(b''.join([(Xtilde.x.val).to_bytes(N_bytes,'big'), (Rsign.x.val).to_bytes(N_bytes,'big'), bytearray(M,'utf-16')])).digest(),'big')%n
        self.c = c
