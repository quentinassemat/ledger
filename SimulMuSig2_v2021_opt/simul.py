...
while i < NB_SIMUL:
    ...
    while j < nb_nonces:
        try:
            client = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            client.connect((ADRESSE, PORT))
            print(bytes(messagePoint(SimulSigners[i].KEY, ((SimulSigners[i].list_r[j]) * G))))
            client.send(
...
#on calcule b
b = [[1] for i in range(NB_SIMUL)]
for l in range(NB_SIMUL):
    b[l] = int.from_bytes(hl.sha256(b''.join([(Xtildes[l].x.val).to_bytes(N_bytes, 'big')] + [(
...
for l in range(NB_SIMUL):
    temp_b = 1
    for j in range(nb_nonces):
        Rsign[l] = Rsign[l].complete_add_unsafe(temp_b * Rn[l][j])
        temp_b = temp_b * b[l] % n
