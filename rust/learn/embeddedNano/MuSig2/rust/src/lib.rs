...
pub fn point_to_bytes_compress(p: ProjectivePoint, compress: bool = true) -> Vec<u8> {
...
pub fn bytes_to_point(bytes: &[u8]) -> ProjectivePoint {
    match EncodedPoint::from_bytes(bytes) {
        Ok(e) => {
            match AffinePoint::from_encoded_point(&e) { //gères compressé et decompréssé je crois
                Some(a) => {
                    return ProjectivePoint::from(a)
                },
                None => {
                    eprintln!("Erreur 1");
        ...
        Err(e) => {
            eprintln!("Erreur ici  : {}", e);
            return ProjectivePoint::generator()
        }
    };
}

pub fn point_to_bytes_decompress(p: ProjectivePoint, ) -> Vec<u8> {
    let a = AffinePoint::from(p);
    let e = EncodedPoint::from(a);
    let c = e.decompress();
    let bytes = c.as_bytes();
